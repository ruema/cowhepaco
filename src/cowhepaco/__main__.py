...
def compare(files, wheel):
    ...
    not_found = set(
        info.filename
        for info in wheel.filelist
        if not info.is_dir()
    ) - names_seen
    for file in not_found:
        errors.append(("additional", file))
    return [(mode, name) for mode, name in errors if not name.split('/')[0].endswith('.dist-info')]
...
def main():
    ...
    filename = os.path.basename(wheel_info["filename"])
